...
export const getServerSideProps = async ({ req, query }) => {
  const productItem = await fetch(
    `http://${req.headers.host}/api/products/product?item=${query.id}`
  )
      ...
      {product ? (
        <div className="w-[90%] h-auto bg-slate-200 flex flex-col relative p-4 rounded-lg">
          ...
          <fieldset className="border-4 border-slate-300 w-full p-6 mt-6">
            ...
            <p className="font-fmate">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit. Molestias fugit consectetur,
              similique corrupti dolore beatae maxime, sapiente dolorum quo illum laboriosam. Vitae
              quod culpa reprehenderit ex quasi totam beatae ipsa.
            </p>
          </fieldset>
