  ...
  return (
    <nav className="bg-amber-500 h-16 w-full md:flex justify-between items-center m-0 fixed z-[500] md:px-5">
      ...
      <div className="flex gap-10">
        <ul
          className={`flex flex-col justify-center items-center gap-5 font-fvolkhov w-full overflow-y-hidden transition-height ease-in delay-150 bg-gray-400 md:bg-transparent md:flex-row md:h-auto ${
            isNavOpen ? 'h-48' : 'h-0'
          }`}
        >
          <li>
          ...
          <li>{!logged ? <Link href="/auth">Login</Link> : <LogoutButton>Logout</LogoutButton>}</li>
        </ul>
